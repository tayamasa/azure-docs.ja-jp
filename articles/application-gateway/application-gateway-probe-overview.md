...
| 異常のしきい値 |3 |通常の正常性プローブで障害が発生した場合に送信するプローブの数を制御します。 これらの正常性プローブは、バックエンドの正常性をすばやく検知するために、プローブの interval 期間を待つことなく立て続けに送信されます。 プローブの連続失敗回数が異常のしきい値に達すると、バックエンド サーバーは「ダウン」とマークされます。 |
